...
@Component({
  selector: 'app-store-teclados',
  templateUrl: './store-teclados.component.html',
  styleUrls: ['./store-teclados.component.css'],
  animations: [
    trigger('foobar', [
...
export class StoreTecladosComponent implements OnInit {
  productsfilter!: any;
  ...
  obtenerproductoshttpfiltered() {
    this.conexion.getproducts().subscribe(products => {
      this.productsfilter = products;
       this.filteredProducts =[...this.productsfilter.filter((user: { category:{name: string | any[];} }) => user.category.name.includes('Teclados USB'))];
       this.state= 'show';
    });
